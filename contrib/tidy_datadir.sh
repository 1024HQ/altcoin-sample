...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete edit_namecoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no edit_namecoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old edit_namecoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected edit_namecoin 0.7 datadir."
    ;;
  3)
    echo "Detected edit_namecoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected edit_namecoin 0.8 datadir."
    ;;
esac
