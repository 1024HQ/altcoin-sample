...
Run with the -testnet option to run with "play 2edit_namecoin2s" on the test network, if you
...
edit_namecoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- BitcoinMiner : Generates 2edit_namecoin2s (if wallet is enabled).
...
However, in open source software such as edit_namecoin Core, where everyone uses
...
pay attention to for reviewers of edit_namecoin Core code.

General edit_namecoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: edit_namecoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of edit_namecoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against edit_namecoin Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:2edit_namecoin2-project/2edit_namecoin2.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `2edit_namecoin2-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `2edit_namecoin2-cli` and the GUI debug console use this table to determine how to
