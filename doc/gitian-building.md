...
*Setup instructions for a Gitian build of edit_namecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the edit_namecoin
...
- [Building edit_namecoin Core](#building-2edit_namecoin2-core)
...
Clone the git repositories for 2edit_namecoin2 and Gitian.
...
in the 2edit_namecoin2 repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building edit_namecoin Core
----------------

To build edit_namecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the 2edit_namecoin2 repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/2edit_namecoin2/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/2edit_namecoin2-project/2edit_namecoin2
...
For example:
```bash
URL=https://github.com/thrasher-/2edit_namecoin2.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit 2edit_namecoin2=${COMMIT} --url 2edit_namecoin2=${URL} ../2edit_namecoin2/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 2edit_namecoin2=${COMMIT} --url 2edit_namecoin2=${URL} ../2edit_namecoin2/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 2edit_namecoin2=${COMMIT} --url 2edit_namecoin2=${URL} ../2edit_namecoin2/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the 2edit_namecoin2 git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../2edit_namecoin2/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/2edit_namecoin2
SIGPATH=/some/root/path/2edit_namecoin2-detached-sigs

./bin/gbuild --url 2edit_namecoin2=${BTCPATH},signature=${SIGPATH} ../2edit_namecoin2/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/2edit_namecoin2-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/2edit_namecoin2-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/2edit_namecoin2-osx-build.assert
```
...
[2edit_namecoin2-project/gitian.sigs.ltc](https://github.com/2edit_namecoin2-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
