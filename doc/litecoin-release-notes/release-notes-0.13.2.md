edit_namecoin Core version 0.13.2 is now available from:

  <https://download.2edit_namecoin2.org/2edit_namecoin2-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/2edit_namecoin2-project/2edit_namecoin2/issues>
...
updates will be released anymore. Without security updates, using a 2edit_namecoin2
...
ECDSA signatures inside edit_namecoin transactions now use validation using
...
Previous versions of edit_namecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of edit_namecoin Core was to raise the default minimum
relay fee.

edit_namecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
edit_namecoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running 2edit_namecoin2d without having to do any manual configuration.
...
edit_namecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. edit_namecoin Core will relay transactions with
...
entirely in edit_namecoin Core 0.13, and it is at this time undecided whether the
...
edit_namecoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
edit_namecoin Core automatically creates a hidden service to listen on, without
manual configuration. edit_namecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if edit_namecoin Core is listening, and
...
edit_namecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals edit_namecoin
Core to use floating fees. By default, edit_namecoin Core will use floating
fees.
...
(default: `0.0002` edit_abbreviation/kB).

At all times, edit_namecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) edit_abbreviation.
Furthermore, edit_namecoin Core will never create transactions paying less than
...
`2edit_namecoin2.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /2edit_namecoin2rpc>
        ...
        # optional bypass 2edit_namecoin2d rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< 2edit_namecoin2rpc:<password>
    </Location>
...
Restarting 2edit_namecoin2d will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in 2edit_namecoin2.conf
...
2edit_namecoin2-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/2edit_namecoin2-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The edit_namecoin Core code base has
...
- `2edit_namecoin2-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `2edit_namecoin2-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the edit_namecoin scripting system that in combination with
...
edit_namecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of edit_namecoin Core.
...
The two phases are distinct in the edit_namecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming edit_namecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for edit_namecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each edit_namecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of 2edit_namecoin2s they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of 2edit_namecoin2s being spent is exactly
...
- **Increased security for multisig:** edit_namecoin addresses (both P2PKH addresses
  ...
  believed to be provided by edit_namecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of edit_namecoin's current ledger (called the UTXO set),
  ...
  note that edit_namecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  edit_namecoin users to individually opt-in to almost any change in the edit_namecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and edit_namecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the edit_namecoin
client to edit_namecoin Core 0.13.2 to signal for segwit support. When a
...
Since edit_namecoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `2edit_namecoin2-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `2edit_namecoin2-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
